...
// 当前登录用户ID，仅本人笔记展示编辑/删除入口（本页为全站笔记广场）
const myId = String(JSON.parse(localStorage.getItem('user') || 'null')?.id || '')
const isMyNote = (row) => String(row.userId) === myId

// 可见性：0-公开 1-仅好友 2-仅自己，后端已按当前身份过滤，列表只会出现可见笔记
const dialogVisible = ref(false)
const saving = ref(false)
const isEdit = ref(false)
const form = ref({ id: null, title: '', content: '', category: '', visibility: 0 })
...
const openCreate = () => {
  isEdit.value = false
  form.value = { id: null, title: '', content: '', category: '', visibility: 0 }
...
const openEdit = (note) => {
  ...
  form.value = {
    ...
    visibility: Number(note.visibility || 0),
  }
  dialogVisible.value = true
}
...
<template>
  <div class="page">
    <AppHeader title="笔记" show-nav />

    <div class="page-container">
      <div class="page-head">
        <div>
          <h2 class="page-title">笔记</h2>
          <p class="page-desc">看看大家与宠物在一起的每个瞬间 · 发布新笔记可为宠物 +10 经验</p>
        </div>
      ...
      <div class="table-wrap pv-panel">
        ...
        >
          ...
          <el-table-column label="作者" width="140">
            <template #default="{ row }">
              <span class="author-link" @click="gotoProfile(row.userId)">{{ row.authorUsername }}</span>
            </template>
          </el-table-column>
      ...
      <el-form :model="form" label-width="70px">
        ...
        <el-form-item label="可见性">
          ...
          <div class="visibility-tip">
            公开：所有人可见；仅好友：只有好友可见；仅自己：只有自己可见（自己始终能看到自己的笔记）
          </div>
        </el-form-item>
      </el-form>
      <template #footer>
